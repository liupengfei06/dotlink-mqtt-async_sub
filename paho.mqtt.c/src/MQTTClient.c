...
#define BUILD_TIMESTAMP "2016ƒÍ 2‘¬25»’ –«∆⁄Àƒ 15 ±51∑÷35√Î CST"
...
int MQTTClient_setup_with_productkey_and_did(char* productkey, char *did, char *productSecret, char *host, int port, char *path, PRO_info *info)
{
    char json_data[1024];
    char sign[33];
    if (productkey == NULL || did==NULL || productSecret==NULL){
        return -11;
    }
    ...
    sprintf(encrypt,"%sdid%sproductkey%s%s",productSecret, did, productkey ,productSecret);
    ...
    sprintf(json_data,"{\"did\": \"%s\", \"productkey\": \"%s\", \"sign\": \"%s\"}", did, productkey, sign);
    
    int ret = http_post_json(json_data, host, port, path, pro_cb);
    if (ret < 0)
        return -1;
